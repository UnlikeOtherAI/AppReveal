...
internal fun registerWebViewTools() {
    ...
    router.register(
        MCPToolDefinition(
            ...
            handler = { params ->
                val js =
                    params?.get("javascript")?.asString
            ...
            handler = { params ->
                ...
                try {
                    MainThreadExecutor.runBlocking {
                        val webView =
                            WebViewBridge.resolveWebView(webViewId)
...
private fun webViewError(message: String): JsonElement = JsonObject().apply { addProperty("error", message) }
