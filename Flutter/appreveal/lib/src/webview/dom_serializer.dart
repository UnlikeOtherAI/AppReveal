// JavaScript strings for DOM operations, injected into WebView instances.
// Identical JS payload to iOS and Android implementations.

abstract final class DOMSerializer {

  // MARK: - DOM Tree

  static String dumpTreeJS({String? root, int maxDepth = 30, bool visibleOnly = false}) {
    final rootExpr = root != null
        ? "document.querySelector('${_escapeJs(root)}')"
        : 'document.body';
    return """
  ...
  static String queryJS(String selector, {int limit = 50}) => """
    (function() {
        var nodes = document.querySelectorAll('${_escapeJs(selector)}');
        var results = [];
  ...
  static String findTextJS(String text, {String? tag}) {
    final tagSelector = tag ?? '*';
    return """
    (function() {
        var search = '${_escapeJs(text)}'.toLowerCase();
        var candidates = document.querySelectorAll('${_escapeJs(tagSelector)}');
        var results = [];
  ...
  static String clickJS(String selector) => """
    (function() {
        var el = document.querySelector('${_escapeJs(selector)}');
        if (!el) return JSON.stringify({error:'Element not found: ${_escapeJs(selector)}'});
        el.click();
  ...
  static String typeJS(String selector, String text, {bool clear = false}) => """
    (function() {
        var el = document.querySelector('${_escapeJs(selector)}');
        if (!el) return JSON.stringify({error:'Element not found: ${_escapeJs(selector)}'});
        el.focus();
        ...
        if (setter && setter.set) {
            setter.set.call(el, ${clear ? "''" : 'el.value'} + '${_escapeJs(text)}');
        } else {
            el.value = ${clear ? "''" : 'el.value'} + '${_escapeJs(text)}';
        }
  ...
  static String selectJS(String selector, String value) => """
    (function() {
        var el = document.querySelector('${_escapeJs(selector)}');
        if (!el || el.tagName !== 'SELECT') return JSON.stringify({error:'Select not found: ${_escapeJs(selector)}'});
  ...
  static String toggleJS(String selector, bool checked) => """
    (function() {
        var el = document.querySelector('${_escapeJs(selector)}');
        if (!el) return JSON.stringify({error:'Element not found: ${_escapeJs(selector)}'});
  ...
  static String scrollToJS(String selector) => """
    (function() {
        var el = document.querySelector('${_escapeJs(selector)}');
        if (!el) return JSON.stringify({error:'Element not found: ${_escapeJs(selector)}'});
  ...
  static String textContentJS({String? selector}) {
    final rootExpr = selector != null
  ...
  static String formsJS() => r"""
    (function() {
        ...
        if (forms.length === 0) {
            ...
            if (inputs.length > 0) {
                var fields = [];
                for (var i = 0; i < inputs.length; i++) { fields.push(describeField(inputs[i])); }
            ...
            for (var f = 0; f < forms.length; f++) {
                ...
                var fields = [];
                for (var i = 0; i < inputs.length; i++) { fields.push(describeField(inputs[i])); }
                results.push({id: form.id || null, action: form.action || null, method: (form.method || 'get').toUpperCase(), fields: fields});
            }
        }
  ...
  static String summaryJS() => r"""
    (function() {
        ...
        document.querySelectorAll('form').forEach(function(f) {
            forms.push({id: f.id || null, action: f.action || null, fieldCount: f.querySelectorAll('input,textarea,select').length});
        });
    ...
    (function() {
        ...
        for (var t = 0; t < tables.length; t++) {
            ...
            table.querySelectorAll('tbody tr, tr').forEach(function(tr) {
                ...
                var cells = [];
                tr.querySelectorAll('td').forEach(function(td) { cells.push(td.textContent.trim().substring(0, 200)); });
                if (cells.length > 0) rows.push(cells);
            });
            results.push({id: table.id || null, headers: headers, rows: rows, rowCount: rows.length});
        }
