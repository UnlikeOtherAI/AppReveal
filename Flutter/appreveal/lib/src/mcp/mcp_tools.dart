...
void registerBuiltInTools() {
  ...
  router.register(
    MCPToolDefinition(
      name: 'get_view_tree',
      description:
          'Dump the full widget hierarchy of the current screen. Returns every widget with type, frame, properties, and depth. Use for discovering all objects on screen.',
      inputSchema: {
        'type': 'object',
        'properties': {
          'max_depth': {
            'type': 'integer',
            'description': 'Max hierarchy depth (default 50)',
          },
        },
      },
    ...
    MCPToolDefinition(
      ...
      inputSchema: {
        ...
        'properties': {
          'element_id': {
            'type': 'string',
            'description': 'Optional element ID to crop to',
          },
          'format': {
            'type': 'string',
            'enum': ['png', 'jpeg'],
  ...
  router.register(
    MCPToolDefinition(
      ...
      handler: (params) async {
        ...
        if (x == null || y == null) {
          return {'error': 'x and y are required numeric coordinates'};
        }
    ...
    MCPToolDefinition(
      ...
      inputSchema: {
        ...
        'properties': {
          ...
          'element_id': {
            'type': 'string',
            'description': 'Optional target element ID',
          },
        },
        'required': ['text'],
      },
  ...
  router.register(
    MCPToolDefinition(
      name: 'scroll',
      description:
          'Scroll a container in a direction. container_id accepts explicit keys or derived IDs from get_elements.',
      inputSchema: {
        'type': 'object',
        'properties': {
          'direction': {
          ...
          'container_id': {
            'type': 'string',
            'description': 'Optional scroll container ID',
          },
        },
        'required': ['direction'],
      },
  ...
  router.register(
    MCPToolDefinition(
      ...
      description:
          'Scroll until an element is visible. Accepts explicit or derived element IDs from get_elements.',
      inputSchema: {
        'type': 'object',
        'properties': {
          'element_id': {'type': 'string'},
  ...
  router.register(
    MCPToolDefinition(
      name: 'batch',
      description:
          'Execute multiple tool calls in a single request. Actions run sequentially. Each action can have an optional delay_ms (milliseconds to wait BEFORE executing that action) to account for animations, screen transitions, or loading. Returns results for every action.',
      inputSchema: {
        'type': 'object',
        'properties': {
          'actions': {
            'type': 'array',
            'description':
                'Array of actions. Each: {"tool": "tool_name", "arguments": {...}, "delay_ms": 500}',
            'items': {
              'type': 'object',
              'properties': {
                ...
                'delay_ms': {
                  'type': 'integer',
                  'description': 'Milliseconds to wait before this action',
                },
              },
              'required': ['tool'],
            },
          ...
          'stop_on_error': {
            'type': 'boolean',
            'description':
                'Stop executing remaining actions if one fails (default: false)',
          },
        },
        'required': ['actions'],
      },
