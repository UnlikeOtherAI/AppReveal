...
internal fun registerBuiltInTools() {
    ...
    router.register(MCPToolDefinition(
        name = "tap_element",
        description = "Tap an element by its ID (accessibility identifier, resource name, or derived ID). Falls back to text-based matching if direct ID lookup fails. Use tap_text if you know the visible text instead of the element ID.",
        inputSchema = jsonSchema(
        ...
        handler = { params ->
            ...
            try {
                ...
                if (success) {
                    ...
                    errorResult("Element not found: $elementId. Try tap_text if you know the visible text.")
                }
            }
        }
    ))
    ...
    router.register(MCPToolDefinition(
        ...
        inputSchema = jsonSchema(),
        handler = { _ ->
            val context = ScreenResolver.appContext
...
internal fun JsonElement.asFloatOrNull(): Float? {
    return if (isJsonPrimitive && asJsonPrimitive.isNumber) asFloat else null
}
